        ...
        if( !commit ) {
            ...
            if( newPassword==null ) {
                ...
                try {
                    ...
                    curator.setData().forPath(ZkPath.CONFIG_ENSEMBLE_PASSWORD.getPath(), PasswordEncoder.encode(newPassword).getBytes("UTF-8"));
                } finally {
                    curator.close();
                }

                // Refresh the default profile to cause all nodes to pickup the new password.
                for (Version version : fabricService.getVersions()) {
                ...
                System.out.println("get delivered as a config update to all the fabric nodes. Once, all the ");
                System.out.println("nodes are updated (nodes must be online), please run:");
                System.out.println("");
