...
public final class KarafContainerRegistration extends AbstractComponent implements ContainerRegistration, ConfigurationListener, ConnectionStateListener {
    ...
    private void activateInternal() {
        ...
        try {
            ...
            if (openshiftEnv) {
                setData(curator.get(), CONTAINER_LOCAL_IP.getPath(karafName), System.getenv("OPENSHIFT_FUSE_IP"));
                setData(curator.get(), CONTAINER_PUBLIC_IP.getPath(karafName), HostUtils.getLocalIp());
            } else {
    ...
    private void registerJmx(Container container) throws Exception {
        ...
        try {
            lock = fabricService.get().getPortService().acquirePortLock();
            ...
            fabricService.get().getPortService().registerPort(container, MANAGEMENT_PID, RMI_REGISTRY_BINDING_PORT_KEY, rmiRegistryPort, lock);
        } finally {
            fabricService.get().getPortService().releasePortLock(lock);
        }
        try {
            lock = fabricService.get().getPortService().acquirePortLock();
            rmiServerPort = getRmiServerPort(container, lock);
            rmiServerConenctionPort = getRmiServerConnectionPort(container, rmiServerPort);
            fabricService.get().getPortService().registerPort(container, MANAGEMENT_PID, RMI_SERVER_BINDING_PORT_KEY, rmiServerPort, lock);
        } finally {
            fabricService.get().getPortService().releasePortLock(lock);
        }
        ...
        String serviceUrl = String.format("service:jmx:rmi://%s:%d/jndi/rmi://%s:%d/karaf-%s",
                rmiServerHost, rmiServerPort, rmiRegistryHost, rmiRegistryPort, karafName);
    ...
    private void registerSsh(Container container) throws Exception {
        ...
        try {
            ...
            fabricService.get().getPortService().registerPort(container, SSH_PID, SSH_BINDING_PORT_KEY, sshPort, lock);
        } finally {
            fabricService.get().getPortService().releasePortLock(lock);
        }
    ...
    private void registerHttp(Container container) throws Exception {
        ...
        if (httpEnabled) {
            try {
                ...
                fabricService.get().getPortService().registerPort(container, HTTP_PID, HTTP_BINDING_PORT_KEY, httpPort, lock);
            } finally {
                fabricService.get().getPortService().releasePortLock(lock);
            }
            if (configuration != null) {
        ...
        if (httpsEnabled) {
            try {
                ...
                fabricService.get().getPortService().registerPort(container, HTTP_PID, HTTPS_BINDING_PORT_KEY, httpsPort, lock);
            } finally {
                fabricService.get().getPortService().releasePortLock(lock);
            }
    ...
    private int getOrAllocatePortForKey(Container container, String pid, String key, int defaultValue, PortService.Lock lock) throws IOException, KeeperException, InterruptedException {
        ...
        Set<Integer> unavailable = fabricService.get().getPortService().findUsedPortByHost(container, lock);
        ...
        if (isValid()) {
            try {
                ...
                if (event.getPid().equals(HTTP_PID) && event.getType() == ConfigurationEvent.CM_UPDATED) {
                    ...
                    int httpConnectionPort = httpsEnabled && !httpEnabled ? getHttpsConnectionPort(current, httpPort) : getHttpConnectionPort(current, httpPort);
